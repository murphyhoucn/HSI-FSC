...
def train(im_width, im_height, depth):
    ...
    torch.save(feature_encoder.state_dict(),str('../modify/meta_training_feature_encoder_' +str(n_way) + 'way_' + str(n_shot) + 'shot_newmodel.pkl'))
    torch.save(relation_network.state_dict(),str('../modify/meta_training_relation_network_' +str(n_way) + 'way_' + str(n_shot) + 'shot_newmodel.pkl'))

    f = open('../modify/meta_training_loss_' +str(n_way) + 'way_' + str(n_shot) + 'shot.txt', 'w')
    for i in range(np.array(loss_).shape[0]):
        f.write(str(loss_[i]) + '\n')
    f = open('../modify/meta_training_accuracy_' +str(n_way) + 'way_' + str(n_shot) + 'shot.txt', 'w')
