...
def metrics(prediction, target, ignored_labels=[], n_classes=None, model_name='none', dataset_name='none'):
    ...
    if mode == 'random':
       ...
       test_gt[test_indices] = gt[test_indices]
        ...
        for i in range(gt.max()):
            index_nd_list.append([])
        # print(index_nd_list)
        # print(len(index_nd_list))
        for i in range(gt.shape[0]):
            for j in range(gt.shape[1]):
                for x in range(gt.max() + 1):
                    if x + 1 == gt[i][j]:
                        index_nd_list[x].append((i, j))
        # print(len(index_nd_list[0]))  ## 标签为1（除了没标记的外，第一个标记类别的个数）
        sample_5_index_nd_list = []
        for i in range(len(index_nd_list)):
            tmp_index = random.sample(index_nd_list[i], 5)
            # print(tmp_index)
            sample_5_index_nd_list.append(tmp_index)
        # print(sample_5_index_nd_list)
        result_list = []
        for i in range(len(sample_5_index_nd_list)):
            for j in sample_5_index_nd_list[i]:
                result_list.append(j)
        print(result_list)
        for i in range(gt.shape[0]):
            for j in range(gt.shape[1]):
                if (i, j) in result_list:
                    train_gt[i][j] = gt[i][j]
                else:
                    train_gt[i][j] = 0
        test_gt = gt
