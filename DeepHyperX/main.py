...
for run in range(N_RUNS):
    if TRAIN_GT is not None and TEST_GT is not None:
        ...
        for i in range(train_gt.shape[0]):
            for j in range(train_gt.shape[1]):
                if train_gt[i][j] != test_gt[i][j]:
                    count111 += 1
                    # print(train_gt[i][j], test_gt[i][j])
                    ## 从这里可以看出来！train_gt部分样本的标签被设置为0了！！
        print(count111)

        ## 查看类别对应的标记样本数
        hs_dict = {}
        hs_dict = hs_dict.fromkeys([i for i in range(N_CLASSES)], 0)
        # print(hs_dict)
        # print(train_gt.max(), train_gt.min())

        for i in range(train_gt.shape[0]):
            for j in range(train_gt.shape[1]):
                if train_gt[i][j] in hs_dict:
                    hs_dict[train_gt[i][j]] += 1
        print(hs_dict)
        print(sum(hs_dict.values()))

        hs_dict = hs_dict.fromkeys([i for i in range(N_CLASSES)], 0)
        # print(hs_dict)
        # print(test_gt.max(), test_gt.min())

        for i in range(test_gt.shape[0]):
            for j in range(test_gt.shape[1]):
                if test_gt[i][j] in hs_dict:
