...
viz.line([[0.5]], [0], win='training loss', opts=dict(title='loss', legend=['loss']))
viz.line([[0.]], [0], win='training acc', opts=dict(title='acc', legend=['acc']))
...
parser.add_argument("-e","--episode",type = int, default= 10000)   # 测试
...
class CNNEncoder(nn.Module):
    ...
    def forward(self,x):
        ...
        out2_layer4 = self.layer4_conv_2_2(self.layer4_conv_2_1(out))
        # print(out2_layer4.size())
        out3_layer4 = self.layer4_conv_3_2(self.layer4_conv_3_1(out))
        # print(out3_layer4.size())
        out4_layer4 = self.layer4_conv_4_2(self.layer4_conv_4_1(out))
...
class RelationNetwork(nn.Module):
    ...
    def forward(self,x): 
        ...
        # print('size x:', list(x.size()))  # size x: [7600, 256, 5, 5]

        out = self.layer1(x)
        # print('out = layer1(x)\nsize out:', list(out.size())) # size out: [7600, 128, 3, 3]

        out = self.layer2(out)
        # print('out = layer2(x)\nsize out:', list(out.size())) # size out: [7600, 64, 1, 1]

        out = out.view(out.size(0),-1) # flatten
        # print('out -> flatten\nsize out:', list(out.size()))  # size out: [7600, 64]

        out = F.relu(self.fc1(out))
        # print('out -> fc1\nsize out:', list(out.size()))  # size out: [7600, 16]

        out = self.dropout(out)
        out = torch.sigmoid(self.fc2(out))
        # print('out -> fc2\nsize out:', list(out.size())) # size out: [7600, 1]
...
def train(im_width, im_height, depth):
    ...
    for episode in range(EPISODE):
        ...
        # print(list(relations.size())) # [7600, 1]
        relations = relations.view(-1, n_way)
        # print(list(relations.size())) # [380, 20]
        ...
        if (episode+1)%10 == 0:  # 设置输出速度
            ...
            # print(type(disp_loss))
            disp_acc = accuracy
            viz.line([[disp_loss]], [episode+1], win='training loss', update='append')
            viz.line([[disp_acc]], [episode+1], win='training acc', update='append')
    ...
    torch.save(feature_encoder.state_dict(),str('../model_modify/meta_training_smalldataset_feature_encoder_' +str(n_way) + 'way_' + str(n_shot) + 'shot_newmodel.pkl'))
    torch.save(relation_network.state_dict(),str('../model_modify/meta_training_smalldataset_relation_network_' +str(n_way) + 'way_' + str(n_shot) + 'shot_newmodel.pkl'))

    f = open('../result_modify/meta_training_smalldataset_loss_' +str(n_way) + 'way_' + str(n_shot) + 'shot.txt', 'w')
    for i in range(np.array(loss_).shape[0]):
        f.write(str(loss_[i]) + '\n')
    f = open('../result_modify/meta_training_smalldataset_accuracy_' +str(n_way) + 'way_' + str(n_shot) + 'shot.txt', 'w')
