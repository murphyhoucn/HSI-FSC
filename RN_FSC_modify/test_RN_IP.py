...
parser.add_argument("-w","--n_way",type = int, default = 16)                      # way  SA_nway
...
feature_encoder.load_state_dict(torch.load(str("../model/IP_feature_encoder_20way_1shot_newmodel_5FT_2000.pkl")))
print("load feature encoder success")

relation_network.load_state_dict(torch.load(str("../model/IP_relation_network_20way_1shot_newmodel_5FT_2000.pkl")))
...
def test(im_width, im_height, channels):
    ...
    #A = time.time()
    # 加载支撑数据
    f = h5py.File('../h5dataset_ica_bandselect_200/IP_' + str(im_width) + '_' + str(im_height) + '_' + str(channels) + '_support' + str(args.n_shot) + '.h5', 'r')
    ...
    # 加载测试
    f = h5py.File(r'../h5dataset_ica_bandselect_200/IP_28_28_100_test.h5', 'r')  # 路径
...
################################################################
    ...
    # n = 42776 # UP  9类标签
    # n = 68877 # XZ   9类标签
    n = 10249    # IP的测试标签数量 16类标签
    matrix = np.zeros((16, 16), dtype=np.int)    # SA 16类标签 
    ...
    path = '../result/' + 'IP_label_test_predict_' + nowtime_style +'.csv'
    ...
    kappa_temp, aa_temp, ca_temp = kappa(matrix, 16) # 16代表 SA的标记样本类别数量

    print(kappa_temp * 100)
    f = open('../result/IP_' + nowtime_style +'.txt', 'w')
    ...
    # SA_colors = ['black', 'gray', 'lime', 'cyan', 'forestgreen', 'hotpink', 'saddlebrown',
    ...
    # cmap = mpl.colors.ListedColormap(SA_colors)
    # plt.xticks([])
    # plt.yticks([])
    # plt.imshow(new_show, cmap=cmap)
    # plt.savefig("../result/SA_" + str(str(np.sum(np.trace(matrix)) / float(n) * 100)) + ".png", dpi=1000)  # 保存图像
