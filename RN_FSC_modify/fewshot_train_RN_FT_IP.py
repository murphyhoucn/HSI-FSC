...
import visdom
viz = visdom.Visdom()
viz.line([[0.5]], [0], win='fewshot_IP_loss', opts=dict(title='fewshot_IP_loss', legend=['fewshot_IP_loss']))
viz.line([[0.]], [0], win='fewshot_IP_acc', opts=dict(title='fewshot_IP_acc', legend=['fewshot_IP_acc']))
...
parser.add_argument("-e","--episode",type = int, default= 3000)   # episode 原来是1000，为了看过程，这里改成1
...
def train(im_width, im_height, depth):
    ...
    # 训练数据集
    f = h5py.File('../h5dataset_ica_bandselect_200/IP_' + str(im_width) + '_' + str(im_height) + '_' + str(depth) + '_support' + str(n_examples) + '.h5', 'r')
    ...
    for episode in range(EPISODE):
        ...
        if (episode+1)%10 == 0:
            ...
            # print(type(disp_loss))
            disp_acc = accuracy
            viz.line([[disp_loss]], [episode+1], win='fewshot_IP_loss', update='append')
            viz.line([[disp_acc]], [episode+1], win='fewshot_IP_acc', update='append')

    print(time.time()-A)

    torch.save(feature_encoder.state_dict(),str('../model/IP_feature_encoder_20way_1shot_newmodel_' + str(n_examples) + 'FT_' + str(EPISODE) + '.pkl'))
    torch.save(relation_network.state_dict(),str('../model/IP_relation_network_20way_1shot_newmodel_' + str(n_examples) + 'FT_' + str(EPISODE) + '.pkl'))
