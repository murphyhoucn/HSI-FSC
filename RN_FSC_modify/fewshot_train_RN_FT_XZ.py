...
import visdom
viz = visdom.Visdom()
viz.line([[0.5]], [0], win='fewshot_XZ_loss', opts=dict(title='fewshot_XZ_loss', legend=['fewshot_XZ_loss']))
viz.line([[0.]], [0], win='fewshot_XZ_acc', opts=dict(title='fewshot_XZ_acc', legend=['fewshot_XZ_acc']))
...
parser.add_argument("-w","--n_way",type = int, default = 9)                      # way XZ
...
def train(im_width, im_height, depth):
    ...
    # 训练数据集
    f = h5py.File('../h5dataset_ica_bandselect_200/XZ_' + str(im_width) + '_' + str(im_height) + '_' + str(depth) + '_support' + str(n_examples) + '.h5', 'r')
    ...
    for episode in range(EPISODE):
        ...
        if (episode+1)%100 == 0:
            ...
            # print(type(disp_loss))
            disp_acc = accuracy
            viz.line([[disp_loss]], [episode+1], win='fewshot_XZ_loss', update='append')
            viz.line([[disp_acc]], [episode+1], win='fewshot_XZ_acc', update='append')

    print(time.time()-A)

    torch.save(feature_encoder.state_dict(),str('../model/XZ_feature_encoder_20way_1shot_newmodel_' + str(n_examples) + 'FT_' + str(EPISODE) + '.pkl'))
    torch.save(relation_network.state_dict(),str('../model/XZ_relation_network_20way_1shot_newmodel_' + str(n_examples) + 'FT_' + str(EPISODE) + '.pkl'))
