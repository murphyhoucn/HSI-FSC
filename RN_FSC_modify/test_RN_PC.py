...
parser.add_argument("-w","--n_way",type = int, default = 9)                      # way PC_nway
...
feature_encoder.load_state_dict(torch.load(str("../model/PC_feature_encoder_20way_1shot_newmodel_5FT_1000.pkl")))
print("load feature encoder success")

relation_network.load_state_dict(torch.load(str("../model/PC_relation_network_20way_1shot_newmodel_5FT_1000.pkl")))
...
def test(im_width, im_height, channels):
    ...
    #A = time.time()
    # 加载支撑数据
    f = h5py.File('../h5dataset_ica_bandselect_200/PC_' + str(im_width) + '_' + str(im_height) + '_' + str(channels) + '_support' + str(args.n_shot) + '.h5', 'r')
    ...
    # 加载测试
    f = h5py.File(r'../h5dataset_ica_bandselect_200/PC_28_28_100_test.h5', 'r')  # 路径
    ...
    # S1
    for i in range(0, 14815):#10988 42776 10249 54129
        # 148140 - 148150
    ...
    # S2
    # 148150 - 148152
    test_images_ = test_images[-2:, :, :, :, :]
    predict_label = rn_predict(support_images, test_images_, num = 2)
    predict_labels.extend(predict_label.cpu().numpy().tolist())
    ...
    print(matrix)
    OA = np.sum(np.trace(matrix)) / 148152.0 * 100
    print('OA = ', round(OA, 2))
...
################################################################
    # n = 54129  # SA的测试标签数量  16类
    n = 148152 # PC  9类
    # n = 42776 # UP 9类
    # n = 68877 # XZ  9类

    matrix = np.zeros((9, 9), dtype=np.int)    # PC  9类
    ...
    path = '../result/' + 'PC_label_test_predict' + nowtime_style +'.csv'
    ...
    kappa_temp, aa_temp, ca_temp = kappa(matrix, 9) # 9代表 PC的标记样本类别数量
    print(kappa_temp * 100)
    f = open('../result/PC_' + nowtime_style + '.txt', 'w')
    ...
    gt = loadmat('../dataset/6_Pavia_Center/Pavia_gt.mat')['pavia_gt']
    ...
    # PC_colors = ['black', 'gray', 'lime', 'cyan', 'forestgreen', 'hotpink', 'saddlebrown',
    ...
    # cmap = mpl.colors.ListedColormap(PC_colors)
    # plt.xticks([])
    # plt.yticks([])
    # plt.imshow(new_show, cmap=cmap)
    # plt.savefig("../result/PC_" + str(str(np.sum(np.trace(matrix)) / float(n) * 100)) + ".png", dpi=1000)  # 保存图像
