...
def train(im_width, im_height, depth):
    ...
    # 训练数据集
    f = h5py.File('../h5dataset_ica_bandselect_200/PC_' + str(im_width) + '_' + str(im_height) + '_' + str(depth) + '_support' + str(n_examples) + '.h5', 'r')
    ...
    torch.save(feature_encoder.state_dict(),str('../model/PC_feature_encoder_20way_1shot_newmodel_' + str(n_examples) + 'FT_' + str(EPISODE) + '.pkl'))
    torch.save(relation_network.state_dict(),str('../model/PC_relation_network_20way_1shot_newmodel_' + str(n_examples) + 'FT_' + str(EPISODE) + '.pkl'))
